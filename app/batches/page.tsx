      ...
      <div className="min-h-screen bg-linear-to-br from-blue-50 to-indigo-100 dark:bg-none dark:bg-background">
        <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          <div className="flex justify-between items-center mb-6">
            <div>
              <h2 className="text-2xl font-bold mb-2">修改批次</h2>
              <p className="text-default-500">
                共 {data?.pagination.total || 0} 个批次
              </p>
            </div>
            <Button
              as={Link}
          ...
          <div className="grid gap-4">
            {data?.batches.map((batch) => (
              <Card key={batch.id} isPressable as={Link} href={`/batches/${batch.id}`}>
                  ...
                  <div className="flex-1">
                    ...
                    {batch.sourceIssue && (
                      <p className="text-small text-primary mt-1">
                        关联 Issue: #{batch.sourceIssue.id} {batch.sourceIssue.title}
                      </p>
                    )}
                  </div>
                </CardHeader>
